...
phyloseq_obj_abundances_filtered <- subset_samples(phyloseq_obj, # new object
                                    current_land_use == "grassland" |
                                    current_land_use == "till cropland" |
                                    current_land_use == "no till cropland")

phyloseq_table <- psmelt(phyloseq_obj_abundances_filtered) %>%
filter(Abundance < 600)
ggplot(phyloseq_table, aes(x = current_land_use, y = Abundance)) + geom_col() + ggtitle("Sequence abundance of current land use")
...
# top-10 Families
prevdf2 <- subset(prevdf, Prevalence >= 20)
ggplot(prevdf2, (aes(x = Order,
                     y = Prevalence,
                     color = Phylum,
                     fill = Family))) +
  geom_col() +
  xlab("Order") +
  ylab("Prevalence") +
  ggtitle("Top 10 Family of bacteria most prevalent in samples") +
